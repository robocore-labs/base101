...
picoros_res_t picoros_interface_init(picoros_interface_t* ifx) {
    ...
    // [axon patch] zp_start_read_task/zp_start_lease_task only exist in
    // multi-threaded builds of this zenoh-pico version; single-threaded
    // operation drives rx/keepalive via picoros_single_threaded_loop().
...
bool picoros_interface_is_up(void) {
    // [axon patch] task-running queries only exist in multi-threaded builds
    #if Z_FEATURE_MULTI_THREAD == 1
    return ( zp_read_task_is_running(z_session_loan(&s_wrapper))
    || zp_lease_task_is_running(z_session_loan(&s_wrapper)) );
    #else
    return !z_session_is_closed(z_session_loan(&s_wrapper));
    #endif
}
